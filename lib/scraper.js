...
async function analyzeAndGeneratePrompts(scraped, brandName, clusters = '') {
  ...
  const prompt = `Analyze this website AND generate 1 search prompt in a single response.
...
PROMPT RULES (exactly 1 prompt — this is a demo scan):
1. The prompt MUST naturally produce a LIST of DIRECT competitor companies to "${brandName}" — same core services, same target market
2. Be hyper-specific to the brand's industry and services — generic industry prompts attract unrelated brands
3. Prefer a fast-to-answer "best/top" framing ("best X for Y", "top X providers") — comparison/alternative prompts take longer to generate and slow the scan
4. Do NOT mention "${brandName}" — generic discovery query
5. Pick the single HIGHEST-VALUE prompt a real buyer would ask AI when evaluating vendors in this exact space
